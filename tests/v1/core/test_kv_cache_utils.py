...
def test_warmup_memory_buffer_scales_with_activation_peak():
    ...
    assert kv_cache_utils._warmup_memory_buffer(0) == floor
    # Below the crossover the floor dominates; above it the peak does.
    small = int(floor / fraction) // 2
    assert kv_cache_utils._warmup_memory_buffer(small) == floor
    large = int(floor / fraction) * 4
    assert kv_cache_utils._warmup_memory_buffer(large) == int(fraction * large)
