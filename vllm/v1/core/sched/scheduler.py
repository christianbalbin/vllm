...
class Scheduler(SchedulerInterface):
    ...
    def update_from_output(
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            # Pre-commit grammar filter: when spec-decode x reasoning
            # produces post-boundary tokens that were sampled WITHOUT the
            # grammar mask engaged (the mask is gated on reasoning_ended,
            # which is False until the boundary is observed - i.e. until
            # after this step's tokens were sampled), drop the
            # grammar-rejected trailing tokens before they are committed
            # and fed to accept_tokens below. Mirrors the
            # verifier-rejected-spec-token bookkeeping above: decrement
            # num_computed_tokens / num_output_placeholders by the
            # rejection count so the next step re-runs from the truncated
            # position with the mask correctly engaged.
            if new_token_ids:
